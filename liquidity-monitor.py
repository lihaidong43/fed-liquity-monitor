# 文件名: liquidity_monitor_real_scale_final.py
# 近十年历史图表 = 2025 当年显示方式（真实值 + 真实Y轴）！
...
def make_chart(start_year):
    ...
    df['BTC']       = btc_full.reindex(dates)  # 2015-2017 为空
    ...
    yaxis_titles = ["美元", "万亿美元", "万亿美元", "万亿美元", "万亿美元", "%", "基点"]
    data_list = [df['BTC'], df['Liquidity'], df['WALCL'], df['TGA'], df['RRP'], df['SOFR'], df['SOFR_Spread']]
    ...
    for i, series in enumerate(data_list):
        ...
        fig.add_trace(go.Scatter(x=clean.index, y=clean.values, name=titles[i], 
                                line=dict(color=colors[i], width=3)), row=i+1, col=1)
        fig.update_yaxes(title_text=yaxis_titles[i], row=i+1, col=1)

    # X轴：每月刻度 + 年份倾斜45° + 最新日期高亮
    monthly_ticks = pd.date_range(f"{start_year}-01-01", datetime.now(), freq='MS')
    latest_date = dates[-1]
    if latest_date not in monthly_ticks:
        monthly_ticks = monthly_ticks.union([latest_date])
    ...
    fig.update_layout(
        height=2800,
        title=f"{'2025 当年日度监控' if start_year == 2025 else '2015-至今近十年历史趋势（真实值）'}"
    )
    return fig

# ==================== Tab ====================
tab1, tab2 = st.tabs(["2025 当年", "2015-至今历史（真实值）"])
...
st.caption(f"数据更新时间：{datetime.now().strftime('%Y-%m-%d %H:%M')} | 历史图表：真实值显示 · 每月刻度 · 年份倾斜45°")